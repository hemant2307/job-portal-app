...
<header>
	<nav class="navbar navbar-expand-lg navbar-light bg-white shadow py-3">
		<div class="container">
			<a class="navbar-brand" href="{{ route('home.page') }}">CareerVibe</a>
			...
			<div class="collapse navbar-collapse" id="navbarSupportedContent">
				<ul class="navbar-nav ms-0 ms-sm-0 me-auto mb-2 mb-lg-0 ms-lg-4">
					...
					<li class="nav-item">
						<a class="nav-link" aria-current="page" href="{{ route('home.job') }}">Find Jobs</a>
					</li>										
				</ul>
				@if (!Auth::check())
  ...
  <div class="modal-dialog modal-dialog-centered">
    <div class="modal-content">
      ...
      <div class="modal-body">
        <form id="profilePicForm" action="" method="POST" name="profilePicForm">
            <div class="mb-3">
                <label for="exampleInputEmail1" class="form-label">Profile Image</label>
                <input type="file" class="form-control" id="image"  name="image">
				<p class="error_image" name= "error_image"></p>
            </div>
	...
	$.ajax({
		...
		success: function (response) {
			if(response.status == false){
				...
				if(errors.image){
                    $(".error_image").text(errors.image[0]);
                  }else{
                    $(".error_image").text("");
                  }
			}else{
