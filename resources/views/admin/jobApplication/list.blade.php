                ...
                <div class="card border-0 shadow mb-4">
                    <div class="card-body card-form">
                        <div class="d-flex justify-content-between">                           
                            <div>
                                <h3 class="fs-4 mb-1">All Jobs applications</h3>
                            </div>
                                            ...
                                            <td>{{ \Carbon\Carbon::parse($application->created_at)->format('d M, Y') }}</td>
                                             <td> 
                                                ...
                                                <ul class="dropdown-menu dropdown-menu-end">
                                                    ...
                                                    {{-- <li><a class="dropdown-item" href="{{ route('admin.user.jobs.edit',$application->id) }}"><i class="fa fa-edit" aria-hidden="true"></i> Edit</a></li> --}}
                                                    <li><a class="dropdown-item" onclick="deleteJob({{ $application->id }})"><i class="fa fa-trash" aria-hidden="true"></i> Delete</a></li>
                                                </ul>
                                            </div>
                                        </td>
...
function  deleteJob(jobId){  
    if(confirm('are you sure you want to delete this Job ?')){
        $.ajax({
            'url' : '{{ route("admin.user.jobapplication.destroy") }}',
            'type': 'delete',
            'data': {jobId : jobId},
            'dataType':  'json',
            success: function(response){
                window.location.href = "{{ route('admin.user.jobapplication') }}";
            }
        });
